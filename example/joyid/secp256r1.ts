...
const calcSignedWitnessLock = (key: EC.KeyPair, tx: CKBTransaction, mode = WITNESS_NATIVE_MODE): Hex => {
...
export const signSecp256r1Tx = (
  ...
  const emptyWitness = {
    ...witnessArgs,
    lock: `0x${calcSignedWitnessLock(key, tx, mode)}`,
  }

  const signedWitnesses = [bytesToHex(blockchain.WitnessArgs.pack(emptyWitness)), ...witnessGroup.slice(1)] as string[]
